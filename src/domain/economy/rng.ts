 ...
 *
 * Implements deterministic seeded RNG compatible with engine canonical RNG.
 * Guaranteed no Math.random() or Date.now() side effects.
 */
...
/**
 * Helper to obtain SeededRng from WorldState without mutating RNG state if the operation is discarded.
 */
export function getRngFromWorld(worldState: any): SeededRng {
  const rngData = worldState?.rng;
  if (!rngData) {
    return new SeededRng(1337);
  }
  if (typeof rngData.serializedState === 'string' && rngData.serializedState.length > 0) {
    try {
      return SeededRng.deserialize(rngData.serializedState);
    } catch {
      // fallback
    }
  }
  if (typeof rngData.seed === 'number') {
    return new SeededRng(rngData.seed, rngData.state, rngData.counter ?? 0);
  }
  return new SeededRng(1337);
}

/**
 * Save updated SeededRng state back to WorldState
 */
export function syncRngToWorld(worldState: any, rng: SeededRng): void {
  if (!worldState.rng) {
    worldState.rng = {
      seed: rng.seed,
    ...
    worldState.rng.seed = rng.seed;
    worldState.rng.counter = rng.drawCount;
    worldState.rng.serializedState = rng.serialize();
  }
}
